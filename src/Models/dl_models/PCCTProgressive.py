...
class PCCTProgressive(BaseDLModel):
    """
    PCCT Progressive V1 - Progressive Cross-Attention CNN-Transformer with Progressive Fusion and Pixel Masking.
    ...
    def __init__(
        ...
        print(f"[PCCT Progressive] Component processing order: {self.component_processing_order}")
        ...
        super().__init__(
            model_type=ModelType.PCCT_PROGRESSIVE,  # Will add to enum
    ...
    def _filter_tokens_by_mask(self, spatial_tokens: tf.Tensor, tomato_mask: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        print(f"[PCCT Progressive V1] Using simplified masking approach")
    ...
    def _get_active_advisor_components(self) -> List[str]:
        ...
        print(f"[PCCT Progressive] Active advisor components: {advisor_components}")
    ...
    def _slice_input_components(self, input_layer: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        for component_name in ['reflectance', 'std', 'ndsi', 'indexes']:
            if (self.components.get(component_name, False) and 
                ...
                print(f"[PCCT Progressive] {component_name}: channels {start_offset}:{end_offset} ({num_channels} channels)")
    ...
    def _build_reflectance_backbone(self, reflectance_input: tf.Tensor) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Progressive] Building CNN backbone with input shape: {reflectance_input.shape}")
        ...
        print(f"[PCCT Progressive] CNN output shape: {x.shape}, creating {N_prime} spatial tokens")
    ...
    def _build_component_encoder(self, component_input: tf.Tensor, component_name: str) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Progressive] Building component encoder for {component_name} with input shape: {component_input.shape}")
        ...
        print(f"[PCCT Progressive] Component {component_name} representation shape: {component_repr.shape}")
    ...
    def _progressive_cross_attention(self, spatial_tokens: tf.Tensor, 
        ...
        """
        print(f"[PCCT Progressive] Applying progressive cross-attention")
        ...
        for i, component_name in enumerate(self.component_processing_order):
            if component_name in component_representations:
                print(f"[PCCT Progressive] Stage {i+1}: Applying cross-attention for {component_name}")
                ...
                print(f"[PCCT Progressive] After {component_name}: tokens shape {current_tokens.shape}")
        
        print(f"[PCCT Progressive] Final enhanced tokens shape: {current_tokens.shape}")
    ...
    def _build_model_for_attr(self) -> tf.keras.Model:
        ...
        """
        print(f"[PCCT Progressive] Building single-attribute model")
        print(f"[PCCT Progressive] Model shape: {self.model_shape}")
        print(f"[PCCT Progressive] Components: {self.components}")
        print(f"[PCCT Progressive] Component dimensions: {self.component_dimensions}")
        ...
        if 'reflectance' not in component_slices:
            raise ValueError("PCCT Progressive requires reflectance component for CNN backbone")
        ...
        print(f"[PCCT Progressive V1] Token filtering: {spatial_tokens.shape} → {filtered_tokens.shape}")
        ...
        # 7. Apply progressive cross-attention (sequential component processing)
        if component_representations:
            enhanced_tokens = self._progressive_cross_attention(spatial_tokens, component_representations)
        else:
            print("[PCCT Progressive V1] Warning: No advisor components active. Using spatial tokens only.")
        ...
        # 12. Create model
        model = Model(inputs=input_layer, outputs=prediction, name="PCCTProgressive")
        ...
        print(f"[PCCT Progressive] Model built successfully with {model.count_params()} parameters")
    ...
    def _build_multi_output_model(self) -> tf.keras.Model:
        ...
        """
        print(f"[PCCT Progressive] Building multi-output model for all attributes: {self.predicted_attributes}")
        ...
        model._name = "PCCTProgressiveMultiRegression"
        
        print(f"[PCCT Progressive] Multi-output model built with {len(self.predicted_attributes)} outputs")
