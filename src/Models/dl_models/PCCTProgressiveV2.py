...
class PCCTProgressiveV2(BaseDLModel):
    """
    PCCT Progressive V2 - Progressive Cross-Attention CNN-Transformer with Progressive Fusion and Spatial Downsampling.
    
    Architecture:
    1. Single CNN backbone processes reflectance data → spatial tokens
    2. Spatial downsampling: Reduce spatial resolution for efficiency (like CDAT V2)
    3. Component encoders process other components → global component representations  
    4. Progressive cross-attention: Components attend sequentially, building upon previous results
    ...
    def __init__(
        ...
        # Progressive fusion parameters
        use_residual_connections: bool = True,
        progressive_scaling: float = 0.1,  # Scale factor for progressive updates
        ...
        # Store PCCT Progressive specific parameters
        ...
        print(f"[PCCT Progressive V2] Component processing order: {self.component_processing_order}")
        print(f"[PCCT Progressive V2] Downsampling: {downsampling_method} with factor {downsampling_factor}")
        ...
        super().__init__(
            model_type=ModelType.PCCT_PROGRESSIVE_V2,  # Will add to enum
    ...
    def _apply_downsampling_layer(self, spatial_features: tf.Tensor, name_prefix: str) -> tf.Tensor:
        """
        Apply spatial downsampling to reduce computational complexity.
        """
        if self.downsampling_method == "strided_conv":
            # Learnable downsampling with strided convolution
            downsampled = Conv2D(
                filters=spatial_features.shape[-1],
                kernel_size=self.downsampling_factor,
                strides=self.downsampling_factor,
                padding='same',
                name=f"{name_prefix}_strided_downsample"
            )(spatial_features)
        elif self.downsampling_method == "max_pool":
            # Fixed max pooling downsampling
            downsampled = MaxPooling2D(
                pool_size=self.downsampling_factor,
                strides=self.downsampling_factor,
                padding='same',
                name=f"{name_prefix}_max_downsample"
            )(spatial_features)
        elif self.downsampling_method == "avg_pool":
            # Fixed average pooling downsampling
            downsampled = AveragePooling2D(
                pool_size=self.downsampling_factor,
                strides=self.downsampling_factor,
                padding='same',
                name=f"{name_prefix}_avg_downsample"
            )(spatial_features)
        else:
            raise ValueError(f"Unknown downsampling method: {self.downsampling_method}")
        
        print(f"[PCCT Progressive V2] Downsampling {spatial_features.shape} → {downsampled.shape}")
        return downsampled
    ...
    def _get_active_advisor_components(self) -> List[str]:
        ...
        print(f"[PCCT Progressive V2] Active advisor components: {advisor_components}")
    ...
    def _slice_input_components(self, input_layer: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        for component_name in ['reflectance', 'std', 'ndsi', 'indexes']:
            if (self.components.get(component_name, False) and 
                ...
                print(f"[PCCT Progressive V2] {component_name}: channels {start_offset}:{end_offset} ({num_channels} channels)")
    ...
    def _build_reflectance_backbone(self, reflectance_input: tf.Tensor) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Progressive V2] Building CNN backbone with input shape: {reflectance_input.shape}")
        ...
        print(f"[PCCT Progressive V2] CNN output shape: {x.shape}, creating {N_prime} spatial tokens")
    ...
    def _build_component_encoder(self, component_input: tf.Tensor, component_name: str) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Progressive V2] Building component encoder for {component_name} with input shape: {component_input.shape}")
        ...
        print(f"[PCCT Progressive V2] Component {component_name} representation shape: {component_repr.shape}")
        return component_repr

    def _progressive_cross_attention(self, spatial_tokens: tf.Tensor, 
                                   component_representations: Dict[str, tf.Tensor]) -> tf.Tensor:
        """
        Apply progressive cross-attention where components attend sequentially, 
        building upon previous results.
        """
        print(f"[PCCT Progressive V2] Applying progressive cross-attention")
        ...
        for i, component_name in enumerate(self.component_processing_order):
            if component_name in component_representations:
                print(f"[PCCT Progressive V2] Stage {i+1}: Applying cross-attention for {component_name}")
                
                component_repr = component_representations[component_name]
                
                # Expand component representation to serve as queries
                ...
                # Cross-attention: component queries attend to current enhanced tokens
                cross_attention_output = MultiHeadAttention(
                    ...
                    key=current_tokens,           # (B, N', d_model) - enhanced tokens from previous stages
                    value=current_tokens          # (B, N', d_model)
                )
                
                # Broadcast component insights to all spatial locations
                component_broadcast = layers.Lambda(
                    lambda x: tf.tile(x[0], [1, tf.shape(x[1])[1], 1]),
                    name=f"{component_name}_broadcast_insights"
                )([cross_attention_output, current_tokens])
                
                # Progressive update with scaling and residual connections
                if self.use_residual_connections:
                    # Scaled addition to prevent overwhelming previous information
                    scaled_component_insights = layers.Lambda(
                        lambda x: self.progressive_scaling * x,
                        name=f"{component_name}_scale_insights"
                    )(component_broadcast)
                    
                    current_tokens = Add(name=f"{component_name}_progressive_add")([
                        current_tokens, scaled_component_insights
                    ])
                else:
                    # Direct addition without scaling
                    current_tokens = Add(name=f"{component_name}_add_insights")([
                        current_tokens, component_broadcast
                    ])
                
                # Layer normalization after each progressive step
                current_tokens = LayerNormalization(
                    epsilon=1e-6, 
                    name=f"{component_name}_progressive_ln"
                )(current_tokens)
                
                print(f"[PCCT Progressive V2] After {component_name}: tokens shape {current_tokens.shape}")
        
        print(f"[PCCT Progressive V2] Final enhanced tokens shape: {current_tokens.shape}")
    ...
    def _add_positional_embeddings(self, spatial_tokens: tf.Tensor) -> tf.Tensor:
        ...
        # Use downsampled dimensions for positional embeddings
        H, W, _ = self.model_shape
        max_positions = (H // self.downsampling_factor) * (W // self.downsampling_factor)
    ...
    def _build_model_for_attr(self) -> tf.keras.Model:
        """
        Build the complete PCCT Progressive model for single attribute prediction.
        """
        print(f"[PCCT Progressive V2] Building single-attribute model")
        print(f"[PCCT Progressive V2] Model shape: {self.model_shape}")
        print(f"[PCCT Progressive V2] Components: {self.components}")
        print(f"[PCCT Progressive V2] Component dimensions: {self.component_dimensions}")
        ...
        if 'reflectance' not in component_slices:
            raise ValueError("PCCT Progressive V2 requires reflectance component for CNN backbone")
        ...
        # 4. Add positional embeddings (downsampling already applied in backbone)
        spatial_tokens = self._add_positional_embeddings(spatial_tokens)
        
        # 5. Build component encoders for advisor components
        ...
        # 6. Apply progressive cross-attention (sequential component processing)
        if component_representations:
            enhanced_tokens = self._progressive_cross_attention(spatial_tokens, component_representations)
        else:
            print("[PCCT Progressive V2] Warning: No advisor components active. Using spatial tokens only.")
            enhanced_tokens = spatial_tokens
        
        # 7. Create CLS token
        cls_token = self._create_learnable_cls_token(input_layer)
        
        # 8. Concatenate CLS token with enhanced spatial tokens
        all_tokens = Concatenate(axis=1, name="concatenate_cls_and_tokens")([cls_token, enhanced_tokens])
        
        # 9. Self-attention layers for global integration
        ...
        # 10. Extract CLS token for prediction
        ...
        # 12. Create model
        model = Model(inputs=input_layer, outputs=prediction, name="PCCTProgressiveV2")
        ...
        print(f"[PCCT Progressive V2] Model built successfully with {model.count_params()} parameters")
        
        return model

    def _build_multi_output_model(self) -> tf.keras.Model:
        """
        Build the PCCT Progressive model for multi-attribute prediction.
        """
        print(f"[PCCT Progressive V2] Building multi-output model for all attributes: {self.predicted_attributes}")
        ...
        model._name = "PCCTProgressiveV2MultiRegression"
        
        print(f"[PCCT Progressive V2] Multi-output model built with {len(self.predicted_attributes)} outputs")
