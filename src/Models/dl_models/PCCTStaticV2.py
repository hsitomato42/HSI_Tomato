...
class PCCTStaticV2(BaseDLModel):
    """
    PCCT Static V2 - Progressive Cross-Attention CNN-Transformer with Static Fusion and Spatial Downsampling.
    ...
    def __init__(
        ...
        # Store PCCT Static V2 specific parameters
        ...
        print(f"[PCCT Static V2] Component processing order: {self.component_processing_order}")
        print(f"[PCCT Static V2] Downsampling: {downsampling_method} with factor {downsampling_factor}")
        ...
        super().__init__(
            model_type=ModelType.PCCT_STATIC_V2,
    ...
    def _apply_downsampling_layer(self, spatial_features: tf.Tensor, name_prefix: str) -> tf.Tensor:
        ...
        print(f"[PCCT Static V2] Downsampling {spatial_features.shape} → {downsampled.shape}")
        return downsampled
    ...
    def _get_active_advisor_components(self) -> List[str]:
        ...
        print(f"[PCCT Static V2] Active advisor components: {advisor_components}")
    ...
    def _slice_input_components(self, input_layer: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        for component_name in ['reflectance', 'std', 'ndsi', 'indexes']:
            if (self.components.get(component_name, False) and 
                ...
                print(f"[PCCT Static V2] {component_name}: channels {start_offset}:{end_offset} ({num_channels} channels)")
    ...
    def _build_reflectance_backbone(self, reflectance_input: tf.Tensor) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Static V2] Building CNN backbone with input shape: {reflectance_input.shape}")
        ...
        print(f"[PCCT Static V2] CNN output shape: {x.shape}, creating {N_prime} spatial tokens")
    ...
    def _build_component_encoder(self, component_input: tf.Tensor, component_name: str) -> tf.Tensor:
        ...
        """
        print(f"[PCCT Static V2] Building component encoder for {component_name} with input shape: {component_input.shape}")
        ...
        print(f"[PCCT Static V2] Component {component_name} representation shape: {component_repr.shape}")
    ...
    def _static_cross_attention(self, spatial_tokens: tf.Tensor, 
        ...
        """
        print(f"[PCCT Static V2] Applying static cross-attention")
        ...
        for component_name, component_repr in component_representations.items():
            print(f"[PCCT Static V2] Applying cross-attention for {component_name}")
        ...
        print(f"[PCCT Static V2] Enhanced tokens shape: {enhanced_tokens.shape}")
    ...
    def _build_model_for_attr(self) -> tf.keras.Model:
        """
        Build the complete PCCT Static V2 model for single attribute prediction.
        """
        print(f"[PCCT Static V2] Building single-attribute model")
        print(f"[PCCT Static V2] Model shape: {self.model_shape}")
        print(f"[PCCT Static V2] Components: {self.components}")
        print(f"[PCCT Static V2] Component dimensions: {self.component_dimensions}")
        ...
        if 'reflectance' not in component_slices:
            raise ValueError("PCCT Static V2 requires reflectance component for CNN backbone")
        ...
        # 6. Apply static cross-attention (all components simultaneously)
        if component_representations:
            enhanced_tokens = self._static_cross_attention(spatial_tokens, component_representations)
        else:
            print("[PCCT Static V2] Warning: No advisor components active. Using spatial tokens only.")
            enhanced_tokens = spatial_tokens
        ...
        # 12. Create model
        model = Model(inputs=input_layer, outputs=prediction, name="PCCTStaticV2")
        ...
        print(f"[PCCT Static V2] Model built successfully with {model.count_params()} parameters")
        
        return model

    def _build_multi_output_model(self) -> tf.keras.Model:
        """
        Build the PCCT Static V2 model for multi-attribute prediction.
        """
        print(f"[PCCT Static V2] Building multi-output model for all attributes: {self.predicted_attributes}")
        ...
        model._name = "PCCTStaticV2MultiRegression"
        
        print(f"[PCCT Static V2] Multi-output model built with {len(self.predicted_attributes)} outputs")
